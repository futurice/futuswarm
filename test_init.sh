...
function ensure_container_running {
    if [ -z "$UP" ]; then
        ...
        docker run -d --privileged --restart always --name worker-1 --hostname=worker-1 -p 127.0.0.1:12375:2375 -p 127.0.0.1:$SSH_PORT_WORKER:22 docker:18.02.0-ce-dind >&/dev/null||true
